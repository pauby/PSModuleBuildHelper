...
Describe 'Function Testing - New-GitHubRelease' {
    Context 'Parameters' {
        ...
        It 'should be true if mandatory parameters have not been changed' {
            $mandatoryParams = @( 'Version', 'CommitID', 'ArtifactPath', 'GitHubUsername', `
                    'GitHubRepository', 'GitHubApiKey')
            $result = Get-FunctionParameter -Name 'New-GitHubRelease' | Where-Object { $_.Value.Attributes.Mandatory -eq $true }
